...
class TestSystemHealth(unittest.TestCase):
    ...
    def test_config_loaded(self):
        ...
        try:
            ...
            self.assertNotEqual(
                config.ANTHROPIC_API_KEY, "", "ANTHROPIC_API_KEY is empty"
            )
    ...
    def test_chroma_db_exists(self):
        ...
        try:
            ...
            self.assertTrue(
                chroma_path.exists(), f"ChromaDB path does not exist: {chroma_path}"
            )
            ...
            if sqlite_file.exists():
                self.log_result(
                    "test_chroma_db_exists", "PASS", f"ChromaDB found at {chroma_path}"
                )
            else:
                self.log_result(
                    "test_chroma_db_exists",
                    "WARN",
    ...
    def test_chroma_db_connection(self):
        ...
        try:
            ...
            if (
                ...
                and "course_content" in collection_names
            ):
                self.log_result(
                    "expected_collections", "PASS", "Both collections found"
                )
            else:
                self.log_result(
    ...
    def test_vector_store_initialization(self):
        ...
        try:
            ...
            self.log_result(
                ...
                "PASS",
                (
                    f"Courses: {course_count}, Titles: {course_titles[:3]}..."
                    if course_titles
                    else "No courses"
                ),
            )
            ...
            if course_count > 0:
                self.log_result(
                    "vector_store_data", "PASS", f"{course_count} courses loaded"
                )
            else:
                self.log_result(
                    "vector_store_data", "FAIL", "No courses found in vector store"
                )
    ...
    def test_ai_generator_initialization(self):
        ...
        try:
            ...
            self.assertIsNotNone(
                ai_generator.client, "Anthropic client not initialized"
            )
    ...
    def test_rag_system_initialization(self):
        ...
        try:
            ...
            self.log_result(
                "test_rag_system_initialization", "PASS", f"Analytics: {analytics}"
            )
