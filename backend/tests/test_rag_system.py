...
class TestRAGSystemEndToEnd(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            if not config.ANTHROPIC_API_KEY or self.rag_system.get_course_analytics()["total_courses"] == 0:
    ...
    def _create_mock_rag_system(self):
        ...
        def mock_query(query, session_id=None):
            if "course outline" in query.lower():
                return "Course outline mock response", [{"text": "Course Outline", "link": None}]
            else:
                return "Mock response to: " + query, [{"text": "Test Course - Lesson 1", "link": "https://example.com/lesson1"}]
    ...
    def test_course_outline_query(self):
        ...
        if self.use_real_system:
            self.assertTrue(
                any(keyword in response.lower() for keyword in ["course", "lesson", "instructor"]),
    ...
    def test_session_management(self):
        ...
        response1, sources1 = self.rag_system.query(query1, None)  # No session ID - should create one
    ...
    def test_course_analytics(self):
        ...
        if self.use_real_system:
            ...
            self.assertEqual(len(course_titles), total_courses, "Course count should match titles count")
    ...
    @unittest.skipIf(not config.ANTHROPIC_API_KEY, "API key required for API integration test")
