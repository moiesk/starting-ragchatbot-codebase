...
class TestVectorStore(unittest.TestCase):
    ...
    def test_basic_search_functionality(self):
        ...
        self.assertGreater(len(results.documents), 0, "No results found for 'introduction'")
        
        # Check metadata structure
        if results.documents:
            self.assertEqual(len(results.documents), len(results.metadata), 
                           "Metadata count doesn't match document count")
            
            # Print first result for debugging
            print(f"   First result metadata: {results.metadata[0] if results.metadata else 'None'}")
    ...
    def test_course_name_resolution(self):
        ...
        if len(first_title.split()) > 1:
            ...
            self.assertIsNotNone(resolved_partial, "Partial course name resolution failed")
    ...
    def test_lesson_number_filter(self):
        ...
        for metadata in results.metadata:
            self.assertEqual(metadata.get('lesson_number'), 1,
                           "Result from wrong lesson found")
    ...
    def test_combined_filters(self):
        ...
        results = self.vector_store.search("content", course_name=first_title, lesson_number=0)
    ...
    def test_invalid_course_name(self):
        ...
        self.assertEqual(len(results.documents), 0, "Should not return documents for invalid course")
    ...
    def test_empty_query(self):
        ...
        if results.error:
            print(f"   Empty query returned error (this may be expected): {results.error}")
