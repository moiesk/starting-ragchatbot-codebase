    ...
    def run_all_tests(self):
        ...
        for module_name in self.test_modules:
            ...
            try:
                ...
                print(f"{status} {module_name}: {result.testsRun} tests, {len(result.failures)} failures, {len(result.errors)} errors")
        ...
        self.generate_report(all_results, total_tests, total_failures, total_errors, total_skips)
        
        return all_results
    
    def generate_report(self, all_results, total_tests, total_failures, total_errors, total_skips):
        ...
        # Overall summary
        overall_status = "✅ SYSTEM HEALTHY" if (total_failures == 0 and total_errors == 0) else "❌ ISSUES DETECTED"
        ...
        for module_name, result_data in all_results.items():
            status_icon = "✅" if (result_data['failures'] == 0 and result_data['errors'] == 0) else "❌"
        ...
        for module_name, result_data in all_results.items():
            ...
            if result.failures:
                for test, error in result.failures:
                    ...
                    print(f"   {issue}")
                    print(f"      Error: {error.split('AssertionError:')[-1].strip()[:200]}...")
            ...
            if result.errors:
                for test, error in result.errors:
                    ...
                    critical_issues.append(issue)
                    print(f"   {issue}")
                    print(f"      Error: {str(error).split('Exception:')[-1].strip()[:200]}...")
