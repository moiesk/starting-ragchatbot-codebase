...
class TestCourseSearchTool(unittest.TestCase):
    ...
    def _create_mock_vector_store(self):
        ...
        mock_results = SearchResults(
            documents=[
                "This is lesson 1 content about introduction",
    ...
    def test_tool_definition_structure(self):
        ...
        print(
            f"   Required params: {definition.get('input_schema', {}).get('properties', {}).get('query', {}).get('type')}"
        )
    ...
    def test_execute_basic_query(self):
        ...
        if self.use_real_vector_store:
            self.assertNotIn(
                ...
                result,
                "Should find content for 'introduction'",
            )
    ...
    def test_execute_with_combined_filters(self):
        ...
        first_course = course_titles[0]
        result = self.search_tool.execute(
            "introduction", course_name=first_course, lesson_number=0
        )
    ...
    def test_execute_invalid_course(self):
        ...
        self.assertIn(
            "No course found", result, "Should return error for invalid course"
        )
    ...
    def test_sources_tracking(self):
        ...
        if self.use_real_vector_store and "No relevant content found" not in result:
            self.assertGreater(
                len(sources), 0, "Should track sources for successful search"
            )
